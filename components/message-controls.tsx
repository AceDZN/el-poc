...
export function MessageControls({
  ...
  const filteredMsgs = msgs.filter(msg => {
    ...
    const matchesSearch =
      searchQuery === '' || JSON.stringify(msg).toLowerCase().includes(searchQuery.toLowerCase());
            ...
            <div className='mt-4'>
              <ScrollArea className='h-[80vh]'>
                <Table className='max-w-full'>
                  ...
                  <TableBody>
                    {filteredMsgs.reverse().map((msg, i) => (
                      <TableRow key={i}>
                        <TableCell className='font-medium'>{msg.type}</TableCell>
                        <TableCell className='max-w-full] whitespace-pre-wrap break-words font-mono text-sm'>
