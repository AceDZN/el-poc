...
export default function useWebRTCAudioSession(
  ...
  function sendTextMessage(text: string) {
    ...
    console.log('dataChannelRef - message', dataChannelRef.current);
  ...
  async function handleDataChannelMessage(event: MessageEvent) {
    if (dataChannelRef.current?.readyState !== 'open') {
      console.log('handleDataChannelMessage - Data channel not open');
      return;
    }
    try {
      ...
      switch (msg.type) {
        ...
        case 'response.function_call_arguments.done': {
          ...
          if (fn) {
            ...
            // Execute function but don't await it to prevent audio interruption
            fn(args);

            // Send function output response without waiting
            const response = {
              type: 'conversation.item.create',
              item: {
                type: 'function_call_output',
                call_id: msg.call_id,
                output: JSON.stringify(args), // Send args directly instead of result
              },
            };
            dataChannelRef.current?.send(JSON.stringify(response));
          }
  ...
  function sendMessage(message: string, role: 'user' | 'assistant' | 'system' = 'system') {
    if (dataChannelRef.current?.readyState !== 'open') {
      console.log('sendMessage - Data channel not open');
      return;
    }
